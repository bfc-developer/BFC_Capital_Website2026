...
const transporter = nodemailer.createTransport({
    ...
    pool: true, // ✅ Reuse connections instead of opening a new one each time
    auth: {
...
export async function POST(request: Request) {
    try {
        ...
        const clientMail = {
            ...
            to: email,
            subject: `We Received Your Message - BFC Capital`,
            html: getClientEmailHtml(finalName, subject, message),
        };

        const adminMail = {
            from: `"${fromName} System" <${fromEmail}>`,
            to: adminEmail,
            subject: `NEW CONTACT FORM: ${subject || 'General'} - ${finalName}`,
            html: getAdminEmailHtml(finalName, finalMobile, email, subject, message),
        };
...
function getClientEmailHtml(name: string, subject: string, message: string): string {
    return `
...
function getAdminEmailHtml(name: string, mobile: string, email: string, subject: string, message: string): string {
    return `
